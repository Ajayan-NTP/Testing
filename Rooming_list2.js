...
//create the group block
 it('post api test to create the group block', async function({supertest}) {
      ...
      .send(
        {
          "blocks": {
            "blockInfo": {
              "block": {
                "blockDetails": {
                  "blockCode": reservation.blockcode4,
                  "blockName": "Valid",
                  "timeSpan": {
  ...
  it('PUT API Test to Update Block Inventory', async function ({supertest}) {
    await supertest
    .request(reservation.request)
        .put(`/blk/v1/hotels/GRVZZ/blocks/${BlockId}/allocation`)
        .set('Content-Type', reservation['Content-Type1'])
        .set('x-hotelid', reservation.hotelId)
...
it('put api test Cancel the group block', async function({supertest}) {
    await supertest
     .request(reservation.request)
     .put(`/blk/v1/hotels/GRVZZ/blocks/${BlockId}/status`)
      .set('Content-Type', reservation['Content-Type1'])
     .set('x-hotelid', reservation.hotelId)
     .set('x-app-key', reservation['x-app-key'])
     .set('bypass-routing', reservation['bypass-routing'])
    ...
    //     await supertest
    //      .request(reservation.request)
    //      .put(`/blk/v1/hotels/GRVZZ/blocks/${BlockId}`)
    //       .set('Content-Type', reservation['Content-Type1'])
    //      .set('x-hotelid', reservation.hotelId)
    //      .set('x-app-key', reservation['x-app-key1'])
    //      .set('bypass-routing', reservation['bypass-routing'])
    ...
    //           "blockName": "Valid",
    //           "timeSpan": {
    //             "startDate": reservation.arrivalDate1,
    //             "endDate": reservation.departureDate1
    //           },
    //           "blockDates": {
    //             "startDate": reservation.arrivalDate1,
    //             "endDate": reservation.departureDate1
    ...
    it(`Post API Test for check block availability`, async function({ supertest }) {
        await supertest
        .request(reservation.request)
        .get(`/blk/v1/hotels/GRVZZ/blocks/${BlockId}/availability`)
        .query({
          "rooms": 1,
      "roomTypeCount": -1,
      "children": 0,
...
//   }); 
      ...
      after(function () {
        const excelFilePath = path.join(filepath.exceloutputpath1);
        ...
        const sheetName = 'RoominglistIAD2';  // Change this for each program (e.g., 'Program1', 'Program2', etc.)
      
        // Use an object to map sheet names to their corresponding headers
        const headersMap = {
            'RoominglistIAD2': ['BlockID', 'Reservation ID', 'Confirmation ID', 'External Reference ID', 'IHG Confirmation Number', 'External Confirmation Number', 'PMS Confirmation Number'],
