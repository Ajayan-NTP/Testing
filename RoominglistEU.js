...
describe("api Authu Token", function () {
  ...
  it("Put api test to the block allocation", async function ({ supertest }) {
    await supertest
      ...
      .send({
        ...
        _xmlns:
          "http://xmlns.oracle.com/cloud/adapter/REST/SetBlockAllocationRange_Opera/types",
      })
      .expect(200)
      .expect("Content-Type", /json/)
      .then(function (response) {
  ...
  //     await supertest
  //      .request(reservation.request)
  //      .put(`/blk/v1/hotels/GRVEU/blocks/${BlockId}`)
  //       .set('Content-Type', reservation['Content-Type1'])
  //      .set('x-hotelid', reservation.hotelId)
  ...
  it("GET Reservation  GRS", async function ({ supertest }) {
    await supertest
      ...
      .then(function (response) {
        ...
        const ihgConfirmationNumberEntry =
          reservation.reservationIds.confirmationNumbers.find((entry) => entry.ihgConfirmationNumber);
  ...
  after(function () {
    ...
    const sheetName = "BlockRoominglistEU1"; // Change this for each program (e.g., 'Program1', 'Program2', etc.)

    // Use an object to map sheet names to their corresponding headers
    const headersMap = {
      'BlockRoominglistEU1': ['BlockID', 'Reservation ID', 'Confirmation ID', 'External Reference ID', 'IHG Confirmation Number', 'External Confirmation Number', 'PMS Confirmation Number'],
